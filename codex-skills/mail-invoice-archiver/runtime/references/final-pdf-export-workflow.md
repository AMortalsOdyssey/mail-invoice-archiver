...
- Treat OFD as a fallback archival format. If a readable PDF exists or can be obtained for the same invoice, do not place an OFD-derived text page or screenshot into the final merged PDF.
- If only OFD exists, label it clearly as an OFD fallback and preserve the original OFD in the source archive.
...
- Merge only invoice documents. Do not add cover pages, summary pages, QR screenshots, OCR placeholder pages, OFD text dumps, water statements, or backup pages unless the user explicitly requests them.
...
3. Remove non-invoices and user-excluded invoices from the candidate set.
...
- [ ] OFD fallbacks appear only when no better invoice representation exists.
...
- Mention any excluded categories, such as non-invoice statements or QR screenshots, without exposing private invoice details.
- Mention any residual uncertainty, missing files, or fallback conversions.
